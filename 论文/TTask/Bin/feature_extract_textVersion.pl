...
my ($usage) = "perl .pl {FilePath} FilePath [w|c|p] forexample:-w word -c chunk -p pos \nNote: w 表示 wordfeature c 表示chunk p表示pos";
my($FilePath,$ftrword,$ftrpos,$ftrchunk);
GetOptions ( "word=s" => \$ftrword,
			"pos=s" => \$ftrpos,  
				"chunk=s" => \$ftrchunk,  
              "file=s"   => \$FilePath, )     # string            
  or die("$usage\n");
#@ARGV || die $usage;
#my ($tfff) = @ARGV;
...
my ($nerpath) = DealTmWheFile($FilePath);
...
open($outFile_hdl,"> ..\\train\\feature\\tmp.feature") or die "can not append it!\n";
my $j=0;
 for(my $i = 0;$i<=$#WORD;$i++)
 {
	 ...
	 if($ftrword =~ /\bword\b/i)
	 {
		print $outFile_hdl "$word_feature\n";
		next;
	 }
	 if($ftrpos =~ /\bpos\b/i)
	 {
